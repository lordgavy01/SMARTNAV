...
class MyTrainer:
    def __init__(self,filename,clusterFileName,model):
        self.trainDF=pd.read_csv(filename)
        clusterDF=pd.read_excel(clusterFileName)
        self.index_to_cluster={clusterDF['Image Name'][i]:clusterDF['refined_predicted_community'][i] for i in  range(len(clusterDF))}
        self.cleanDataset()
        self.model=model
    ...
    def cleanDataset(self):
        ...
        for i in range(len(self.trainDF)):
            ...
            for j in range(1,51):
                nname=name+str(j)
                depth=self.trainDF.iloc[i][nname]
                if depth==1e9:
                    depths.append(1000)
                else:
                    depths.append(depth)
            inputs['X1'].append(depths.copy())
            inputs['X3'].append([self.index_to_cluster[i]])
        self.loadDataset(inputs)
    
    def loadDataset(self,inputs):
        self.dataset=LidarDataset(inputs)   
        self.loader=torch.utils.data.DataLoader(self.dataset, batch_size=512, shuffle=True,pin_memory=True)
    ...
    def trainDataset(self,checkpointFilename):
        ...
        for epoch in range(Epochs):
            ...
            for batch_index,batch in enumerate(loop):
                ...
                batch_X3 = batch['X3'].to(device)
                batch_Y = batch['Y'].to(device)
                # process
                predicted = self.model(batch_X1,batch_X2,batch_X3)
...
model=Model(50,2,512,512)

trainer=MyTrainer("Datasets/train.csv","Results/Clustering/clustering_0_1.xlsx",model)
trainer.trainDataset('newModel')
